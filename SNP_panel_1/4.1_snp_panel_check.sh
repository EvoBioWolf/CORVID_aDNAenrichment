...
### check and remove non-biallelic sites again, then map to outgroups New Caledonian crow and American crow ###
### final plot ###
...
final="/dss/dsslegfs01/pr53da/pr53da-dss-0018/projects/2020__ancientDNA/01_probes/snp_panel_summary/05_final"
...
awk 'NR>1 {print $3, $4}' $neu/outlier_genic_neutral_$1 > snp_panel_$1_pos.txt
#recall snps from vcf
vcftools --vcf $ref/christen_vcf/genome_crow.gatkHC.vqsrts99.sort5_incl.scaf5.repcont_SNP_ID.recode.vcf \
--positions snp_panel_$1_pos.txt --recode --out snp_panel_$1
...
bedtools getfasta -fi $ref2/genome_HC_allpaths41687_v2.5.fasta -bed snp_panel_$1_trimmed_pos.bed -fo snp_panel_$1_probesize.fa
#some probes need to be trimmed to fit into scaffold length or they will be omitted
#Feature (scaffold_147:220036-220156) beyond the length of scaffold_147 size (220110 bp).  Skipping.
#Feature (scaffold_246:29819-29939) beyond the length of scaffold_246 size (29929 bp).  Skipping.
#Feature (scaffold_761:1731-1851) beyond the length of scaffold_761 size (1832 bp).  Skipping.
#Feature (scaffold_849:1451-1571) beyond the length of scaffold_849 size (1541 bp).  Skipping.
#Feature (scaffold_883:1372-1492) beyond the length of scaffold_883 size (1457 bp).  Skipping.
## sed -i -e 's/220036\t220156/220036\t220110/' snp_panel_105k_set6up_trimmed_pos.bed
## sed -i -e 's/29819\t29939/29819\t29929/' snp_panel_105k_set6up_trimmed_pos.bed
## sed -i -e 's/1731\t1851/1731\t1832/' snp_panel_105k_set6up_trimmed_pos.bed
## sed -i -e 's/1451\t1571/1451\t1541/' snp_panel_105k_set6up_trimmed_pos.bed
## sed -i -e 's/1372\t1492/1372\t1457/' snp_panel_105k_set6up_trimmed_pos.bed
...
## gzip -d GCF_009650955.1_bCorMon1.pri_genomic.fna.gz
## makeblastdb -in GCF_009650955.1_bCorMon1.pri_genomic.fna -dbtype nucl -out bCorMon1
...
## gzip -d GCF_000691975.1_ASM69197v1_genomic.fna.gz
## makeblastdb -in GCF_000691975.1_ASM69197v1_genomic.fna -dbtype nucl -out ASM69197v1
...
wc -l snp_panel_$1_trimmed_mapped_pos.txt #103771
rm *.tmp
...
cd $final
Rscript ../final_plot.R $1 G01_G02 G06_G07 G08_G09 05_final
rm *.gds*
mkdir plots
mv *.pdf *.jpg ./plots
